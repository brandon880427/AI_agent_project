...
(() => {
  ...
  function ensureChatContainer() {
    ...
    if (!chatContainer) {
      ...
      if ($finalList && $finalList.parentElement) {
        ...
        (() => {
          ...
          if (!canvas || !ctx) {
            // If canvas cannot be initialized, nothing else can work.
            return;
          }
          ...
          async function drawArrayBuffer(buf) {
            if (!(buf instanceof ArrayBuffer)) return;
            // If canvas rect is 0 (e.g. tab hidden), re-fit to avoid permanent black.
            fitCanvas();
          ...
          function connectCamera() {
            ...
            camWs.onopen = () => {
              ...
              if (stallTimer) { try { clearInterval(stallTimer); } catch (e) {} }
    ...
    wsCam.onclose = ()=>{
      setBadge($camStatus, false, 'Camera: disconnected');
      _lastFrameAt = 0;
      if (_stallTimer){ try{ clearInterval(_stallTimer); }catch(e){} _stallTimer = null; }
      const delay = Math.min(10000, 500 * Math.pow(2, camRetry));
      camRetry = Math.min(camRetry + 1, 6);
      if (camReTimer){ try{ clearTimeout(camReTimer); }catch(e){} }
      camReTimer = setTimeout(connectCamera, delay);
    };
    wsCam.onerror = ()=>{
      setBadge($camStatus, false, 'Camera: error');
  ...
  async function triggerDescribe(){
    try{
      ...
      if (!r.ok){
        addMessage(`（vision）请求失败：HTTP ${r.status}`, false);
        return;
      }
      if (j && j.ok === false && j.status === 'busy'){
        addMessage('（vision）正在分析中，请稍后再试。', false);
      }
    }catch(e){
      addMessage('（vision）请求失败：' + (e && e.message ? e.message : String(e)), false);
    }
  }
