...
# ===== 非阻塞写入：后台线程 + 有界队列（满了丢最旧） =====
_writer_thread: Optional[threading.Thread] = None
_writer_stop_evt = threading.Event()
_frame_q: Optional["queue.Queue[bytes]"] = None
_audio_q: Optional["queue.Queue[Tuple[bytes, str]]"] = None

# 默认上限：视频帧只保留很少，确保实时；音频允许稍多但仍有上限
_FRAME_Q_MAX = int(os.getenv("RECORDER_FRAME_QUEUE_MAX", "6"))
_AUDIO_Q_MAX = int(os.getenv("RECORDER_AUDIO_QUEUE_MAX", "128"))


def _q_put_drop_oldest(q: "queue.Queue", item) -> None:
    """非阻塞 put：满了就先丢一个最旧的。"""
    try:
        q.put_nowait(item)
        return
...
def _ensure_writer_running() -> None:
    global _writer_thread, _frame_q, _audio_q
    if _writer_thread and _writer_thread.is_alive():
        return

    _writer_stop_evt.clear()
    _frame_q = queue.Queue(maxsize=max(1, _FRAME_Q_MAX))
    _audio_q = queue.Queue(maxsize=max(8, _AUDIO_Q_MAX))

    def _worker():
        recorder = get_recorder()
        while True:
            if _writer_stop_evt.is_set():
                # drain remaining quickly then exit
                drained_any = False
                try:
                    while _frame_q and not _frame_q.empty():
                        drained_any = True
                        try:
                            jpg = _frame_q.get_nowait()
                        except Exception:
                            break
                        try:
                            recorder.add_frame(jpg)
                        except Exception:
                            pass
                    while _audio_q and not _audio_q.empty():
                        drained_any = True
                        try:
                            pcm, txt = _audio_q.get_nowait()
                        except Exception:
                            break
                        try:
                            recorder.add_audio(pcm, txt)
                        except Exception:
                            pass
                finally:
                    if not drained_any:
    ...
    _writer_thread = threading.Thread(target=_worker, daemon=True)
...
def record_frame(jpeg_data: bytes):
    ...
    if recorder.is_recording:
        ...
        if _frame_q is None:
            ...
            _q_put_drop_oldest(_frame_q, jpeg_data)
...
def record_audio(pcm_data: bytes, text: str = ""):
    ...
    if recorder.is_recording:
        ...
        if _audio_q is None:
            ...
            _q_put_drop_oldest(_audio_q, (pcm_data, text))
