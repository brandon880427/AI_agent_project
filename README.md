...
### 🎙️ 实时语音交互
- **语音识别（ASR）**：项目原先集成了阿里云 DashScope Paraformer 作为可选的实时语音识别（若你想继续使用 DashScope，可保留相关配置）。
- **多模态对话（LLM）**：已将对话服务迁移为本地 Ollama（默认 http://localhost:11434），支持将 content 转为 prompt 并调用本地模型生成文本回复。若仍需云端多模态语音合成，请另行集成 TTS 服务。
...
### 外部服务配置
- **Ollama (本地 LLM)**：推荐使用本地 Ollama 作为对话模型，默认地址为 `http://localhost:11434`，模型名通过 `OLLAMA_MODEL` 指定。
- **（可选）DashScope**：如果你使用 DashScope 的 ASR/云服务，请保留或填入 `DASHSCOPE_API_KEY`。否则无需 DashScope API Key。
...
创建 `.env` 文件并配置你的 Ollama 地址与模型（本地 Ollama 示例）：
...
说明：
- 本项目默认使用本地 Ollama 作为 LLM（替代原先的 DashScope Qwen-Omni）。
- 若你仍使用 DashScope 的 ASR 功能（实时语音识别），可以在 `.env` 中保留 `DASHSCOPE_API_KEY`，系统会尽量兼容旧配置。
